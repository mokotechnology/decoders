...
function Decoder(bytes, fPort, groupID) {
    ...
    if (fPort == 1) {
        ...
        var auxiliary_operation = auxiliaryOperationArray[bytes[1] & 0x0F]; //device status
        payloadList.push(getPayloadData('auxiliary_operation', auxiliary_operation, groupID));
        var timezone = timezone_decode(bytes[2]); //timezone
        payloadList.push(getPayloadData('timezone', timezone, groupID));
        var timestamp = bytesToInt(bytes, 3, 4); //timestamp
        payloadList.push(getPayloadData('timestamp', timestamp, groupID));
        var shutdown_type = shutdownTypeArray[bytes[7]];
        ...
        var timestamp = bytesToInt(bytes, 3, 4); //timestamp
        payloadList.push(getPayloadData('timestamp', timestamp, groupID));
        var low_power_prompt_percent = (bytes[7] & 0xFF) + '%'; //low power level
        payloadList.push(getPayloadData('low_power_prompt_percent', low_power_prompt_percent, groupID));
    }
    else if (fPort == 6 || fPort == 10) {
        var device_mode = deviceModeArray[(bytes[1] >> 5) & 0x07 - 1]; //work mode
        payloadList.push(getPayloadData('device_mode', device_mode, groupID));
        var auxiliary_operation = auxiliaryOperationArray[bytes[1] >> 2 & 0x07]; //device status
        payloadList.push(getPayloadData('auxiliary_operation', auxiliary_operation, groupID));
        var age = ((bytes[1] & 0x01) << 8 | bytes[2]).toString() + 's';
        ...
        var auxiliary_operation = auxiliaryOperationArray[bytes[1] & 0x0F]; //device status
        payloadList.push(getPayloadData('auxiliary_operation', auxiliary_operation, groupID));
        var age = ((bytes[2]) << 8 | bytes[3]).toString() + 's'; //age
        payloadList.push(getPayloadData('age', age, groupID));
        var parse_len = 4;
        ...
        var auxiliary_operation = auxiliaryOperationArray[bytes[1] & 0x0F]; //device status
        payloadList.push(getPayloadData('auxiliary_operation', auxiliary_operation, groupID));
        var reasons_for_positioning_failure = posFailedReasonArray2[bytes[2] - 1];
        payloadList.push(getPayloadData('reasons_for_positioning_failure', reasons_for_positioning_failure, groupID));
        parse_len = 3;
